...
import esw.dsl.LocationServiceDsl
...
class LocationServiceUtil(private[esw] val _locationService: LocationService)(implicit protected val actorSystem: ActorSystem[_])
  ...
  def listByComponentName(name: String)(implicit ec: ExecutionContext): Future[List[Location]] = {
    _locationService.list.map { locations =>
  ...
  def resolveByComponentNameAndType(name: String, componentType: ComponentType)(
    ...
    await(_locationService.list(componentType))
  ...
  def resolveComponentRef(componentName: String, componentType: ComponentType)(
    ...
    _locationService.resolve(connection, 10.seconds).map
