...
(along with HCD, Assembly, and Application). The Sequence Component is configured by 
...
and consistent environment for a Sequencer. Scripts are stored in 
...
a Sequencer can receive and process is a Sequence, which can be 1 or more Sequence Commands. This is important to point 
out because a Sequence that is received may be only 1 Sequence Command. 

There is one more term: **Observing Block**. The Observing Block is an SOSS type that is usually defined as the 
"smallest schedulable science entity". The Observing Block is more than the Sequence. The Sequence is extracted 
from the Observing Block before it is sent to the OCS.
...
interactive environment that can be used during engineering and acceptances testing.
...
Sequencer mode as defined in the model files for the Sequencer/Script.
...
Assemblies in their model files. The Sequence Command may trigger a mode or state machine that operates after the 
...
for an observing mode, but the construction agreement is that there will be one Sequencer for each of the major 
...
The following figure shows all the key discussions points of previous paragraphs. This figure shows a simplified 
