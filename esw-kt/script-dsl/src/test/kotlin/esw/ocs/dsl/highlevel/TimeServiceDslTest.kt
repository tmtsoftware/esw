...
class TimeServiceDslTest : WordSpec(), TimeServiceDsl {
    ...
    init {
        "TimeServiceDsl" should {
            ...
            "offsetFromNow should give offset between current time and provided instance of the time | ESW-122" {
            ...
            "utcTimeAfter should give UTC time of after given duration from now | ESW-122" {
                val offset: Duration = utcTimeAfter(1.seconds).offsetFromNow()
                assertWithin(expected = 1.seconds, result = offset, tolerance = 2.milliseconds)
            }

            "taiTimeAfter should give time TAI of after given duration from now | ESW-122" {
