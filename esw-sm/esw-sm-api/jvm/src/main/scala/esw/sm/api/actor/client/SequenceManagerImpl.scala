...
class SequenceManagerImpl(location: AkkaLocation)(implicit actorSystem: ActorSystem[_], timeout: Timeout)
  ...
  override def cleanup(observingMode: String): Future[CleanupResponse] =
    smRef ? (Cleanup(observingMode, _))

  override def getRunningObsModes: Future[GetRunningObsModesResponse] =
    smRef ? GetRunningObsModes
