...
private[ocs] class SequencerWiring(val sequencerId: String, val observingMode: String, sequenceComponentName: Option[String]) {
  ...
  val cswServicesWiring = new CswServicesWiring(actorRuntime, sequenceOperatorFactory)

  lazy val sequencerBehavior =
    new SequencerBehavior(componentId, script, cswServicesWiring.locationService, cswServicesWiring.commandResponseManager)

  lazy val sequencerRef: ActorRef[EswSequencerMessage] = (typedSystem ? Spawn(sequencerBehavior.setup, sequencerName)).block

  private lazy val script: Script = ScriptLoader.load(scriptClass, cswServicesWiring.cswServices)
  ...
  def start(): Either[RegistrationError, AkkaLocation] = {
    ...
    cswServicesWiring.locationServiceUtils.register(registration)(typedSystem).block
  }
}
// $COVERAGE-ON$
