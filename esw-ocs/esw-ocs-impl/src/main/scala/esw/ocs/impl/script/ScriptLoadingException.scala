...
object ScriptLoadingException {
  class ScriptConfigurationMissingException(subsystem: Subsystem, observingMode: ObsMode)
      extends RuntimeException(s"Script configuration missing for [${subsystem.name}] with [${observingMode.name}]")
