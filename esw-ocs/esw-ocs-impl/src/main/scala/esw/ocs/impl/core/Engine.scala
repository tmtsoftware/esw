...
private[ocs] class Engine(implicit mat: Materializer) {
  ...
  def start(sequenceOperator: SequenceOperator, script: JScript): Future[Done] = {
   ...
   */
  private def processStep(sequenceOperator: SequenceOperator, script: JScript): Future[Done] = async {
