...
private[ocs] class Engine(implicit mat: Materializer) {
  ...
  def start(sequenceOperator: SequenceOperator, script: ScriptDsl): Future[Done] = {
   ...
   */
  private def processStep(sequenceOperator: SequenceOperator, script: ScriptDsl): Future[Done] = async {
