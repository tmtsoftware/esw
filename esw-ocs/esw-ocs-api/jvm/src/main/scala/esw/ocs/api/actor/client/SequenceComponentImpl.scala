...
class SequenceComponentImpl(sequenceComponentLocation: AkkaLocation)(implicit
  ...
  override def loadScript(subsystem: Subsystem, observingMode: String): Future[LoadScriptResponse] =
    sequenceComponentRef ? (LoadScript(subsystem, observingMode, _))

  override def restart(): Future[RestartScriptResponse] = sequenceComponentRef ? Restart
