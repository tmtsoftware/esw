...
object SequencerMessages {
  ...
  final case class LoadAndStart(sequence: Sequence, replyTo: ActorRef[SubmitResponse])       extends IdleMessage
  ...
  final case class Available(replyTo: ActorRef[Boolean]) extends IdleMessage with SequenceLoadedMessage with InProgressMessage
