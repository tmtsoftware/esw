...
object ElementConverter {
  ...
  def toMap(input: Dom.MapElem): Map[String, Any] = input.toMap.collect {
    case (Dom.StringElem(k), v) if v != Dom.NullElem => k -> toAny(v)
  }

  def toSeq(input: Dom.ArrayElem): Seq[Any] = input.elements.collect {
  ...
  def fromSeq(input: Seq[Any]): Dom.ArrayElem = Dom.ArrayElem.Unsized {
    input.collect {
