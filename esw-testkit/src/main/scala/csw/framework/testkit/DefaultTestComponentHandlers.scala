...
class DefaultTestComponentHandlers extends TestComponentHandlers {
   ...
   */
  override def initialize(cswCtx: CswContext): Unit = {}
   ...
   */
  override def onLocationTrackingEvent(cswCtx: CswContext, trackingEvent: TrackingEvent): Unit = {}
   ...
   */
  override def onSubmit(cswCtx: CswContext, runId: Id, controlCommand: ControlCommand): CommandResponse.SubmitResponse =
    Completed(runId)
   ...
   * @param controlCommand represents a command received e.g. Setup, Observe or wait
   */
  override def onOneway(cswCtx: CswContext, runId: Id, controlCommand: ControlCommand): Unit = {}
   ...
   */
  override def onDiagnosticMode(cswCtx: CswContext, startTime: UTCTime, hint: String): Unit = {}
   ...
   * @param cswCtx CSW context provided to give CSW access to method
   */
  override def onOperationsMode(cswCtx: CswContext): Unit = {}
   ...
   */
  override def onShutdown(cswCtx: CswContext): Unit = {}
   ...
   * @param cswCtx CSW context provided to give CSW access to method
   */
  override def onGoOffline(cswCtx: CswContext): Unit = { isOnline = false }
   ...
   * @param cswCtx CSW context provided to give CSW access to method
   */
  override def onGoOnline(cswCtx: CswContext): Unit = { isOnline = true }
}
