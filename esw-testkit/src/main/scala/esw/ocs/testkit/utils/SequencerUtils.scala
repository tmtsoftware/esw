...
trait SequencerUtils extends LocationUtils {
  ...
  def spawnSequencerRef(subsystem: Subsystem, observingMode: ObsMode): ActorRef[SequencerMsg] =
    spawnSequencer(subsystem, observingMode).sequencerRef

  def spawnSequencerProxy(subsystem: Subsystem, observingMode: ObsMode): SequencerApi =
    new SequencerImpl(spawnSequencerRef(subsystem, observingMode))

  def spawnSequencer(subsystem: Subsystem, observingMode: ObsMode): AkkaLocation =
    loadScript(spawnSequenceComponent(subsystem, None), subsystem, observingMode)

  def spawnSequencerInSimulation(subsystem: Subsystem, observingMode: ObsMode): AkkaLocation =
    loadScript(spawnSequenceComponentInSimulation(subsystem, None), subsystem, observingMode)

  def sequencerClient(subsystem: Subsystem, observingMode: ObsMode): SequencerApi =
    SequencerApiFactory.make(resolveHTTPLocation(Prefix(subsystem, observingMode.name), ComponentType.Sequencer))
  ...
  private def loadScript(seqCompLocation: AkkaLocation, subsystem: Subsystem, observingMode: ObsMode) =
