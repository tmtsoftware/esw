...
import esw.sm.api.protocol.{ConfigureResponse, ShutdownAllSequencersResponse}
...
class SequenceManagerSossIntegrationTest extends EswTestKit(EventServer) {
  ...
  "SOSS" must {
    "have ability be able to spawn sequencer hierarchy and send sequence to top level sequencer | ESW-146" in {
      ...
      sequenceManager.shutdownObsModeSequencers(obsMode).futureValue shouldBe a[ShutdownAllSequencersResponse.Success.type]
    }
  }
}
