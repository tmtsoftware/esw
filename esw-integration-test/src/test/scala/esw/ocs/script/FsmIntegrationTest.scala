...
class FsmIntegrationTest extends EswTestKit(EventServer) {
  ...
  "Fsm" must {
    ...
    "start child Fsm and accept commands | ESW-246, ESW-251, ESW-142" in {
      ...
      val fsmSequencer: SequencerApi = spawnSequencerProxy(Prefix(ESW, ObsMode("fsm").name))
    ...
    "pass parameters to next state via become | ESW-246, ESW-251, ESW-252, ESW-142" in {
      ...
      val fsmSequencer: SequencerApi = spawnSequencerProxy(Prefix(ESW, ObsMode("becomeFsm").name))
    ...
    "command flag should trigger FSM bind to it | ESW-246, ESW-251, ESW-252, ESW-142" in {
      ...
      val fsmSequencer: SequencerApi = spawnSequencerProxy(Prefix(ESW, ObsMode("commandFlagFsm").name))
    ...
    "be able to bind to param variables with polling time | ESW-142, ESW-256, ESW-291" in {
      val subsystem                  = ESW
      val obsMode                    = ObsMode("MoonNight")
      val fsmSequencer: SequencerApi = spawnSequencerProxy(Prefix(subsystem, obsMode.name))
