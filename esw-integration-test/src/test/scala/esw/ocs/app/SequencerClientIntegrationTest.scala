...
//  LoggingSystemFactory.start("SequencerClientIntegrationTest", "0.1", host, typedSystem)
  ...
  "LoadSequence, Start it and Query its response | ESW-145, ESW-154, ESW-221, ESW-194, ESW-158, ESW-222, ESW-101, ESW-244" in {
    ...
    ocsSequencer.loadSequence(sequence).futureValue should ===(Unhandled(Offline.name, "LoadSequence"))
    val invalidId = Id("IdNotAvailable")

    val invalidStartResponse =
      Invalid(invalidId, UnsupportedCommandInStateIssue(Unhandled(Offline.name, "StartSequence").msg))
...
//    error.message should fullyMatch regex s"StepId: .*, CommandName: ${failedCmdName.name}, reason: Server responded with 500 Internal Server Error"
  ...
  "Go online and offline | ESW-194, ESW-222, ESW-101, ESW-134, ESW-236" in {
    ...
    ocsSequencer.add(List(command3)).futureValue should ===(Unhandled(Offline.name, "Add"))
  ...
  "LoadSequence, Start it and Abort sequence | ESW-155, ESW-137" in {
    ...
    ocsSequencer.abortSequence().futureValue should ===(Unhandled(Loaded.name, "AbortSequence"))
  ...
  "LoadSequence, Start it and Stop | ESW-156, ESW-138" in {
    ...
    ocsSequencer.stop().futureValue should ===(Unhandled(Loaded.name, "Stop"))
