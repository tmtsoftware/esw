...
class SequencerAdminIntegrationTest extends ScalaTestFrameworkTestKit with BaseTestSuite with SequencerAdminHttpCodecs {
  ...
  "Sequencer" must {
    "start the sequencer and handle the http requests | ESW-222, ESW-101" in {
      ...
      val stepList = sequencerAdminClient.getSequence.futureValue.get
      stepList.steps should ===(List(Step(command1), Step(command2)))

      val startSequenceResponse = sequencerAdminClient.startSequence.futureValue
      startSequenceResponse should ===(Ok)

      val sequenceFinalResponse = sequencerAdminClient.queryFinal.futureValue
      sequenceFinalResponse should ===(SequenceResult(Completed(stepList.runId)))
    }
  }
