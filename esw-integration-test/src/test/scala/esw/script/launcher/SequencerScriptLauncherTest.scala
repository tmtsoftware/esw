// XXX TODO FIXME: This test does not work with script HTTP server, since the lauchSequencer.sh script uses the--extra-jars
//  option, which won't work, since script server is another process, started by ocsApp (SequencerApp) via cs!
