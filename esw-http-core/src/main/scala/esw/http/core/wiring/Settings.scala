...
class Settings(_port: Option[Int], _serviceName: Option[String], _config: Config) {
  ...
  lazy val httpConnection: HttpConnection = HttpConnection(ComponentId(serviceName, ComponentType.Service))
}
