...
kotlinc -jvm-target 1.8 -opt-in=kotlin.time.ExperimentalTime -Xallow-any-scripts-in-source-roots -classpath "$(cs fetch --channel $CS_CHANNEL esw-ocs-app$VERSION --classpath)" $FILE_PATH -d $JARNAME
