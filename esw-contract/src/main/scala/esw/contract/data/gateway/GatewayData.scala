...
trait GatewayData extends CommandData with SequencerData {
  ...
  val setAlarmSeverity: SetAlarmSeverity = SetAlarmSeverity(AlarmKey(prefix, "someAlarm"), AlarmSeverity.Okay)
  ...
  val websocketSequencerCommand: WebsocketRequest.SequencerCommand = WebsocketRequest.SequencerCommand(componentId, sequencerQueryFinal)
