...
import esw.agent.pekko.client.AgentCommand.SpawnCommand.SpawnSequenceComponent
...
class ProcessManagerTest extends BaseTestSuite {
  ...
  private val connection = PekkoConnection(compId)
  private val uri        = new URI("some")

  private val processExecutor = mock[ProcessExecutor]
  private val versionManager  = mock[VersionManager]
  ...
  "spawn" must {
    "return failed response when component is already registered | ESW-237, ESW-367" in {
      ...
      val spawnSequenceComponent =
        SpawnSequenceComponent(probe.ref, Prefix("ESW.primary"), "darknight", None, simulation = false, test = true)
  ...
  "kill" must {
    ...
    "return failed response when pid does not exist on agent machine | ESW-276, ESW-367" in {
      val pid      = 12345
      val location = PekkoLocation(connection, uri, Metadata().withPid(pid))
      val manager = new ProcessManager(locationService, versionManager, processExecutor, agentSetting) {
