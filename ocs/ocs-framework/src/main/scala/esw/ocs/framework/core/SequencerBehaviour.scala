...
import esw.ocs.framework.api.models.messages.{SequencerMsg, StepListActionResponse}
...
object SequencerBehaviour {
  def behavior(crmRef: ActorRef[CommandResponseManagerMessage]): Behavior[SequencerMsg] =
    Behaviors.setup { ctx =>
      ...
      def updateAndSendResponse[T <: StepListActionResponse](stepListResult: StepListResult[T], replyTo: ActorRef[T]): Unit = {
