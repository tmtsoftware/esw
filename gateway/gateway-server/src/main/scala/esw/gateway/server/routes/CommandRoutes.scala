...
class CommandRoutes(cswCtx: CswContext) extends JsonSupportExt {
  ...
  val routes: Future[CommandService] => Route = (commandServiceF: Future[CommandService]) =>
  ...
  val route: Route =
    pathPrefix("command") {
      pathPrefix(Segment / Segment) { (componentType, componentName) =>
        componentType match {
          case "assembly" => routes(componentFactory.assemblyCommandService(componentName))
          case "hcd"      => routes(componentFactory.hcdCommandService(componentName))
        }
      }
    }
}
